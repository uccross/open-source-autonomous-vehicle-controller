...
.. module:: Linear.py
	...
	:synopsis: 2D Linear trajectory generation, connecting waypoints for 
    Autonomous an Surface Vehicle (ASV)
...
class Linear():
    ...
    def is_closest_point_beyond_next(self, threshold):
        is_beyond = False
        c0 = self.closPT-self.prevWP
        t0 = self.nextWP-self.prevWP
        
        if np.linalg.norm(c0)+(threshold/2.0) >= np.linalg.norm(t0):
            is_beyond = True
